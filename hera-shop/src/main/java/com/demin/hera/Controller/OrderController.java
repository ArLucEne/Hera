...
public class OrderController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", dataType = "String", name = "buyerId",
                    value = "买家的id"),
            @ApiImplicitParam(paramType = "query", dataType = "Integer", name = "status",
                    value = "查询订单的类型: 0为未支付订单,1为支付成功的订单")})
    public Response list(@PathVariable String buyerId, @PathVariable Integer status) {
    ...
    @ApiImplicitParams({
            ...
            @ApiImplicitParam(paramType = "query", dataType = "long", name = "size",
                    value = "每一页返回的条数")})
    public Response list(@PathVariable String buyerId,
...
/*    @GetMapping("/{buyerId}")
    ...
    public Response groupByStatus(@PathVariable String buyerId) {
